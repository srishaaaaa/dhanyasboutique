...
export default function AddProductModal({ isOpen, onClose, onSuccess }: AddProductModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-shopDeep/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-shopSoft/40">

        <div className="shrink-0 flex items-center justify-between p-6 border-b border-shopSoft/40 bg-[#F9FAFB]">
            ...
            {categoryMode === 'select' ? (
              <div className="flex gap-1">
                <select
                  ...
                  className="flex-1 w-full min-w-0 px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold appearance-none"
                >
                ...
                <button
                  ...
                  className="px-2 py-3 text-[10px] font-black text-shopCard bg-[#F9FAFB] border border-shopSoft/60 rounded-xl hover:bg-shopSoft/40 transition-colors shrink-0"
              ...
              <div className="flex gap-1">
                <input
                  ...
                  value={formData.category}
                  onChange={e => setFormData({...formData, category: e.target.value})}
                  className="flex-1 w-full px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold"
                  placeholder="Type new category"
                />
                <button
                  type="button"
                  onClick={() => { setCategoryMode('select'); setFormData(f => ({...f, category: ''})) }}
                  className="px-2 py-3 text-[10px] font-black text-[#374151] bg-[#F9FAFB] border border-shopSoft/60 rounded-xl hover:bg-shopSoft/40 transition-colors shrink-0"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold text-right"
                placeholder="0"
              />
            </div>
            <div>
