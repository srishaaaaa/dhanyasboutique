...
export default function CatalogModal({ isOpen, onClose, onAdd }: CatalogModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-shopDeep/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-5xl flex min-h-0 flex-col shadow-2xl overflow-hidden border border-shopSoft/40 max-h-[calc(100dvh-1rem)] sm:max-h-[85vh]">

        {editingProduct ? (
          <>
            <div className="flex items-center justify-between p-6 border-b border-shopSoft/40 bg-[#F9FAFB]">
              <h2 className="text-xl font-black text-[#111111]">Edit Product</h2>
              <button onClick={cancelEdit} className="p-2 rounded-xl hover:bg-shopCard/5 text-[#374151]">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={saveEdit} className="min-h-0 flex-1 overflow-y-auto hide-scrollbar p-6 flex flex-col gap-4">
              ...
              <div>
                ...
                <input type="text" value={editForm.name}
                  ...
                  className="w-full px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editForm.category}
                    ...
                    className="w-full min-w-0 h-12 px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold touch-manipulation">
                ...
                <div>
                  ...
                  <input type="number" value={editForm.price}
                    ...
                    className="w-full px-4 py-3 bg-[#F9FAFB] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold text-right" placeholder="0" />
                </div>
              </div>
          ...
          <>
            <div className="flex items-center justify-between p-5 border-b border-shopSoft/40 bg-[#F9FAFB]">
              <h2 className="text-[18px] font-black text-[#111111] flex items-center gap-2">
                <Search size={18} className="text-shopCard" />
                Search Catalog
              </h2>
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-shopCard/5 text-[#374151]">
                <X size={20} />
              </button>
            </div>
            <div className="p-3 sm:p-4 border-b border-shopSoft/40 bg-white space-y-3">
              <div className="relative">
                ...
                <input type="text" value={search}
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-[#FAFAFA] border border-shopSoft/60 rounded-xl focus:outline-none focus:border-shopCard text-[13px] font-bold text-[#111111]" />
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto hide-scrollbar p-3 sm:p-4 bg-[#FAFAFA]">
              {loading ? (
                <div className="flex min-h-48 flex-col items-center justify-center gap-3 text-[#374151]/70">
                  <span className="h-7 w-7 animate-spin rounded-full border-2 border-shopSoft border-t-shopCard" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                  {filtered.map(product => (
                    <div key={product.id}
                      className="bg-white border border-shopSoft/60 rounded-2xl p-3 flex flex-col gap-2 hover:border-shopCard/40 hover:shadow-md transition-all group relative">
                      <div className="flex items-start justify-between gap-2">
                        <h4 onClick={() => onAdd(product)} className="min-w-0 flex-1 cursor-pointer text-[13px] font-black text-[#111111] leading-tight group-hover:text-shopCard transition-colors">{product.name}</h4>
                        <div className="flex shrink-0 gap-1">
                          <button onClick={(e) => { e.stopPropagation(); startEdit(product) }} title="Edit product"
                            className="p-1.5 rounded-lg bg-white border border-shopSoft/60 text-[#374151] hover:text-shopCard hover:border-shopCard/40 shadow-sm transition-colors">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); void handleDelete(product) }} title="Delete product"
                            className="p-1.5 rounded-lg bg-white border border-shopSoft/60 text-red-400 hover:text-red-600 hover:border-red-300 shadow-sm transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
                      ...
                      <div onClick={() => onAdd(product)} className="cursor-pointer">
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-shopSoft/30 gap-2">
