...
function LoadingSpinner() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-bgMain">
      <span className="h-10 w-10 animate-spin rounded-full border-4 border-shopSoft border-t-shopCard" />
    </div>
  )
}
...
function AppShell() {
  ...
  // Load the shop/company profile once so brand details and the card-colour
  // theme are applied everywhere they are already displayed.
  useEffect(() => {
    void fetchSettings()
  }, [fetchSettings])

  useEffect(() => {
    document.title = shopName || BRAND_EN
  }, [shopName])
