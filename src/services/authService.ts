...
const getUsers = (): LocalUser[] => {
  try {
    const raw = localStorage.getItem('ssp_tex_users') || '[]'
...
const saveUsers = (u: LocalUser[]) => localStorage.setItem('ssp_tex_users', JSON.stringify(u))
...
export const authService = {
  ...
  signUp: async (params: {
    ...
    localStorage.setItem('ssp_tex_session', newUser.id)
  ...
  signIn: async (email: string, password: string): Promise<{ user: AuthUser | null; error: string | null }> => {
    ...
    localStorage.setItem('ssp_tex_session', match.id)
  ...
  signOut: async (): Promise<void> => {
    ...
    if (USE_LOCAL_AUTH_FALLBACK) {
      localStorage.removeItem('ssp_tex_session')
    }
  },
  ...
  getCurrentUser: async (): Promise<AuthUser | null> => {
    ...
    // localStorage fallback
    const sid = localStorage.getItem('ssp_tex_session')
    if (!sid) return null
    const users = getUsers()
