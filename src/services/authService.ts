...
const getUsers = (): LocalUser[] => {
  try {
    const raw = localStorage.getItem('dhanyas_users') || '[]'
...
const saveUsers = (u: LocalUser[]) => localStorage.setItem('dhanyas_users', JSON.stringify(u))
...
export const authService = {
  ...
  signUp: async (params: {
    ...
    localStorage.setItem('dhanyas_session', newUser.id)
  ...
  signIn: async (email: string, password: string): Promise<{ user: AuthUser | null; error: string | null }> => {
    ...
    localStorage.setItem('dhanyas_session', match.id)
  ...
  signOut: async (): Promise<void> => {
    ...
    if (USE_LOCAL_AUTH_FALLBACK) {
      localStorage.removeItem('dhanyas_session')
    }
  },
  ...
  getCurrentUser: async (): Promise<AuthUser | null> => {
    ...
    // localStorage fallback
    const sid = localStorage.getItem('dhanyas_session')
    if (!sid) return null
    const users = getUsers()
