...
export function advanceReceiptPdf(order: AdvanceOrder) {
  ...
  doc.setFillColor(resolveCardColorHex()); doc.rect(0, 0, 210, 5, 'F')
  try { doc.addImage(getShopLogoDataUrl() || LOGO_BASE64, 'PNG', 16, 10, 12, 12) } catch (err) { console.warn('Advance receipt: logo image failed to load', err) }
  doc.setTextColor(resolveCardColorHex()); doc.setFont('helvetica', 'bold'); doc.setFontSize(18); doc.text(BRAND_EN.toUpperCase(), 38, 20)
  ...
  doc.setDrawColor(resolveCardColorHex()); doc.line(16, 38, 194, 38)
