...
import { resolveCardColorHex, resolveCardSoftHex } from './shopTheme'
...
export function createInvoicePdf(data: InvoicePdfData): Blob {
  ...
  // Brand colours
  const RED   = resolveCardColorHex()
  const SOFT  = resolveCardSoftHex()
  const DARK  = '#141414'
  const INK   = '#1a1a2e'
  const MUTED = '#555555'
  const LIGHT = '#888888'
  ...
  const logoSize = 20
  try {
    doc.addImage(getShopLogoDataUrl() || LOGO_BASE64, 'PNG', left, y, logoSize, logoSize)
  } catch {
